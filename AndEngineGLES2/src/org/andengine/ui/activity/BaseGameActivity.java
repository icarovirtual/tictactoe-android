...
public abstract class BaseGameActivity extends BaseActivity implements IGameInterface, IRendererListener {
	...
	public synchronized void onSurfaceChanged(final GLState pGLState, final int pWidth, final int pHeight) {
		if (BuildConfig.DEBUG) {
			Debug.d(this.getClass().getSimpleName() + ".onSurfaceChanged(Width=" + pWidth + ",  Height=" + pHeight + ")" + " @(Thread: '"
	...
	protected synchronized void onCreateGame() {
		...
		final OnPopulateSceneCallback onPopulateSceneCallback = new OnPopulateSceneCallback() {
			...
			public void onPopulateSceneFinished() {
				try {
					...
					Debug.e(BaseGameActivity.this.getClass().getSimpleName() + ".onGameCreated failed." + " @(Thread: '"
		...
		final OnCreateSceneCallback onCreateSceneCallback = new OnCreateSceneCallback() {
			...
			public void onCreateSceneFinished(final Scene pScene) {
				...
				try {
					...
					Debug.e(BaseGameActivity.this.getClass().getSimpleName() + ".onPopulateScene failed." + " @(Thread: '"
							+ Thread.currentThread().getName() + "')", pThrowable);
				}
			}
		};
		...
		final OnCreateResourcesCallback onCreateResourcesCallback = new OnCreateResourcesCallback() {
			...
			public void onCreateResourcesFinished() {
				try {
					...
					Debug.e(BaseGameActivity.this.getClass().getSimpleName() + ".onCreateScene failed." + " @(Thread: '"
							+ Thread.currentThread().getName() + "')", pThrowable);
				}
			}
		};
		...
		try {
			...
			Debug.e(this.getClass().getSimpleName() + ".onCreateGame failed." + " @(Thread: '" + Thread.currentThread().getName() + "')",
					pThrowable);
		}
	}
	...
	public synchronized void onGameCreated() {
		...
		if (this.mOnReloadResourcesScheduled) {
			...
			try {
				...
				Debug.e(this.getClass().getSimpleName() + ".onReloadResources failed." + " @(Thread: '" + Thread.currentThread().getName()
						+ "')", pThrowable);
			}
		}
	}
	...
	protected void onDestroy() {
		...
		try {
			...
			Debug.e(this.getClass().getSimpleName() + ".onDestroyResources failed." + " @(Thread: '" + Thread.currentThread().getName()
					+ "')", pThrowable);
		}
		...
		case LANDSCAPE_SENSOR:
			if (SystemUtils.SDK_VERSION_GINGERBREAD_OR_LATER) {
				...
				Debug.w(ScreenOrientation.class.getSimpleName() + "." + ScreenOrientation.LANDSCAPE_SENSOR
						+ " is not supported on this device. Falling back to " + ScreenOrientation.class.getSimpleName() + "."
