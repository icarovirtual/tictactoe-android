...
public class ScreenGrabber extends Entity {
	...
	public void grab(final int pGrabX, final int pGrabY, final int pGrabWidth, final int pGrabHeight,
			final IScreenGrabberCallback pScreenGrabCallback) {
		this.mGrabX = pGrabX;
		this.mGrabY = pGrabY;
	...
	private static Bitmap grab(final int pGrabX, final int pGrabY, final int pGrabWidth, final int pGrabHeight) {
		...
		// TODO Check availability of OpenGL and GLES20.GL_RGBA combinations
		// that require less conversion operations.
		// Note: There is (said to be) a bug with glReadPixels when 'y != 0', so
		// we simply read starting from 'y == 0'.
		...
		// Convert from RGBA_8888 (Which is actually ABGR as the whole buffer
		// seems to be inverted) --> ARGB_8888
