...
/**
 * Note: {@link ButtonSprite} needs to be registered as a {@link ITouchArea} to
 * the {@link Scene} via {@link Scene#registerTouchArea(ITouchArea)}, otherwise
 * it won't be clickable. To make {@link ButtonSprite} function properly, you
 * should consider setting
 * {@link Scene#setTouchAreaBindingOnActionDownEnabled(boolean)} to
 * <code>true</code>.
 * 
 * (c) Zynga 2012
 * 
...
public class ButtonSprite extends TiledSprite {
	...
	public ButtonSprite(final float pX, final float pY, final ITextureRegion pNormalTextureRegion,
		...
		this(pX, pY, new TiledTextureRegion(pNormalTextureRegion.getTexture(), pNormalTextureRegion), pVertexBufferObjectManager,
				pOnClickListener);
	}

	public ButtonSprite(final float pX, final float pY, final ITextureRegion pNormalTextureRegion,
			final ITextureRegion pPressedTextureRegion, final VertexBufferObjectManager pVertexBufferObjectManager) {
	...
	public ButtonSprite(final float pX, final float pY, final ITextureRegion pNormalTextureRegion,
		...
		this(pX, pY, new TiledTextureRegion(pNormalTextureRegion.getTexture(), pNormalTextureRegion, pPressedTextureRegion),
				pVertexBufferObjectManager, pOnClickListener);
	}

	public ButtonSprite(final float pX, final float pY, final ITextureRegion pNormalTextureRegion,
			final ITextureRegion pPressedTextureRegion, final ITextureRegion pDisabledTextureRegion,
			final VertexBufferObjectManager pVertexBufferObjectManager) {
		this(pX, pY, pNormalTextureRegion, pPressedTextureRegion, pDisabledTextureRegion, pVertexBufferObjectManager,
	...
	public ButtonSprite(final float pX, final float pY, final ITiledTextureRegion pTiledTextureRegion,
		...
		case 3:
			break;
		default:
			throw new IllegalArgumentException("The supplied " + ITiledTextureRegion.class.getSimpleName()
	...
	private void changeState(final State pState) {
		...
		if (stateTiledTextureRegionIndex >= this.mStateCount) {
			...
			Debug.w(this.getClass().getSimpleName() + " changed its " + State.class.getSimpleName() + " to " + pState.toString()
					+ ", which doesn't have a " + ITextureRegion.class.getSimpleName() + " supplied. Applying default "
					+ ITextureRegion.class.getSimpleName() + ".");
		} else {
