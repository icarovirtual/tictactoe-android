 ...
 * @see {@link http
 *      ://algorithmist.wordpress.com/2009/10/06/cardinal-splines-part-4/}
 */
public class CardinalSplineMoveModifier extends DurationEntityModifier {
	...
	protected void onManagedUpdate(final float pSecondsElapsed, final IEntity pEntity) {
		...
		/*
		 * Formula: s * (-ttt + 2tt – t) * P1 + s * (-ttt + tt) * P2 + (2ttt –
		 * 3tt + 1) * P2 + s(ttt – 2tt + t) * P3 + (-2ttt + 3tt) * P3 + s * (ttt
		 * – tt) * P4
		 */
		final float s = (1 - this.mCardinalSplineMoveModifierConfig.mTension) / 2;

		final float b1 = s * ((-ttt + (2 * tt)) - t); // (s * (-ttt + 2tt – t))
														// * P1
		final float b2 = (s * (-ttt + tt)) + (((2 * ttt) - (3 * tt)) + 1); // (s
																			// *
																			// (-ttt
																			// +
																			// tt)
																			...
																			// *
																			// P2
		final float b3 = (s * ((ttt - (2 * tt)) + t)) + ((-2 * ttt) + (3 * tt)); // (s
																					// *
																					// (ttt
																					// –
																					// 2tt
	...
	public static final float cardinalSplineX(final float pX0, final float pX1, final float pX2, final float pX3, final float pT,
			final float pTension) {
		final float t = pT;
		final float tt = t * t;
		final float ttt = tt * t;
	...
	public static final float cardinalSplineY(final float pY0, final float pY1, final float pY2, final float pY3, final float pT,
			final float pTension) {
		final float t = pT;
		final float tt = t * t;
		final float ttt = tt * t;
	...
	public static class CardinalSplineMoveModifierConfig {
		...
		public CardinalSplineMoveModifierConfig(final int pControlPointCount, final float pTension) {
			if (pControlPointCount < CardinalSplineMoveModifierConfig.CONTROLPOINT_COUNT_MINIMUM) {
				throw new IllegalArgumentException("A " + CardinalSplineMoveModifierConfig.class.getSimpleName() + " needs at least "
						+ CardinalSplineMoveModifierConfig.CONTROLPOINT_COUNT_MINIMUM + " control points.");
			}
