	...
	public class DebugTime {
		...
		public void dump(final int pIndent, final String pPostfix) {
			if (this.mSplit) {
				...
				Arrays.fill(indent, ' ');
				Debug.log(DebugTimer.this.mDebugLevel, new String(indent) + SPLIT_STRING + "'" + this.mLabel + "'" + " @( "
						+ (this.mEndTime - this.mStartTime) + "ms )" + pPostfix);
			} else {
				...
				if (this.mChildren == null) {
					...
					Debug.log(DebugTimer.this.mDebugLevel, new String(indent) + "}@( " + (this.mEndTime - this.mStartTime) + "ms )"
							+ pPostfix);
				}
			}
		}
