...
/**
 * The Jarvis March algorithm marches around the hull, like a ribbon wrapping
 * itself around the points, this algorithm also called the
 * <i><b>gift-wrapping</b></i> algorithm.
 * 
...
public class JarvisMarch implements IHullAlgorithm {
	...
	@Override
	public int computeHull(final float[] pVertices, final int pVertexCount, final int pVertexOffsetX, final int pVertexOffsetY,
	...
	private static int jarvisMarch(final float[] pVertices, final int pVertexCount, final int pVertexOffsetX, final int pVertexOffsetY,
			final int pVertexStride) {
		...
		do {
			...
			/*
			 * Compute the angle between the current hull vertex and all
			 * remaining vertices. Pick the smallest angle larger then the
			 * current angle.
			 */
