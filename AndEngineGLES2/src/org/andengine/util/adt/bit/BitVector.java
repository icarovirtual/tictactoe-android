...
public final class BitVector {
	...
	private BitVector(final int pCapacity) {
		...
		/*
		 * Check if bytes perfectly fit into the data fields or if there are
		 * some overflow bytes that need special treatment.
		 */
	...
	public BitVector(final byte[] pBytes) {
		...
		for (int i = lastCompleteDataIndex; i >= 0; i--) {
			...
			data[i] = ((((long) pBytes[bytesOffset + 0]) << 56) & 0xFF00000000000000L)
					...
					| ((((long) pBytes[bytesOffset + 3]) << 32) & 0xFF00000000L) | ((((long) pBytes[bytesOffset + 4]) << 24) & 0xFF000000L)
					| ((((long) pBytes[bytesOffset + 5]) << 16) & 0xFF0000L) | ((((long) pBytes[bytesOffset + 6]) << 8) & 0xFF00L)
