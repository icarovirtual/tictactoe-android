 ...
 * of elements that need to be sorted by natural order instead of the order they
 * are queue. Its {@link java.util.List} like behavior performs better than a
 * plain {@link java.util.ArrayList}, since it automatically shift the contents
 * of its internal Array only when really necessary. Besides sparse allocations
 * to increase the size of the internal Array,
 * {@link com.zynga.mobileville.path.SortedList} is allocation free (unlike the
 * {@link java.util.LinkedList} family).
 * 
 * (c) Zynga 2012
 * 
...
public class SortedList<T extends Comparable<T>> implements ISortedList<T> {
	...
	/**
	 * Scans for items around <code>pGuess</code> that fulfill
	 * <code>pItem.compareTo(item) == 0</code> and starting from the leftmost
	 * found, it returns the index of the first one that fulfills
	 * <code>pItem.equals(item)</code>.
	 * 
	 * @param pStart
	 *            left bound.
	 * @param pEnd
	 ...
	 * @param pItem
	 *            to perform <code>pItem.compareTo(item) == 0</code> and
	 *            <code>pItem.equals(item)</code> checks on.
	 * @param pReturnSequenceEndIfNoEqualItemFound
	 * @return
	 */
	private int scanForEqualItem(final int pStart, final int pEnd, final int pGuess, final T pItem,
			final boolean pReturnSequenceEndIfNoEqualItemFound) {
		...
		/*
		 * From the beginning of the sequence, advance until the first item
		 * equals pItem or the end has been reached.
		 */
		while (i < pEnd) {
			final T item = this.mList.get(i);
			if (i <= pGuess) {
				/*
				 * Since the compartTo check has already been performed, only
				 * equals needs to be checked.
				 */
