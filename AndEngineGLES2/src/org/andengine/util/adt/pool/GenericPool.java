...
public abstract class GenericPool<T> {
	...
	/**
	 * @param pItem
	 *            every item that was just obtained from the pool, passes this
	 *            method.
	 */
	...
	public synchronized T obtainPoolItem() {
		...
		if (this.mAvailableItems.size() > 0) {
			...
			if (BuildConfig.DEBUG) {
				Debug.v(this.getClass().getName() + "<" + item.getClass().getSimpleName() + "> was exhausted, with "
						+ this.mUnrecycledItemCount + " item not yet recycled. Allocated " + this.mGrowth + " more.");
			}
		}
