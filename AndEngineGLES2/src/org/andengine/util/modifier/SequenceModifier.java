...
public class SequenceModifier<T> extends BaseModifier<T> implements IModifierListener<T> {
	...
	public SequenceModifier(final ISubSequenceModifierListener<T> pSubSequenceModifierListener, final IModifier<T>... pModifiers)
	...
	public SequenceModifier(final ISubSequenceModifierListener<T> pSubSequenceModifierListener,
			final IModifierListener<T> pModifierListener, final IModifier<T>... pModifiers) throws IllegalArgumentException {
	...
	public float onUpdate(final float pSecondsElapsed, final T pItem) {
		if (this.mFinished) {
			...
			while (secondsElapsedRemaining > 0 && !this.mFinishedCached) {
				secondsElapsedRemaining -= this.mSubSequenceModifiers[this.mCurrentSubSequenceModifierIndex].onUpdate(
						secondsElapsedRemaining, pItem);
			}
			this.mFinishedCached = false;
