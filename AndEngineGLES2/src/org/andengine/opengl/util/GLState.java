...
public class GLState {
	...
	public void reset(final RenderOptions pRenderOptions, final ConfigChooser pConfigChooser, final EGLConfig pEGLConfig) {
		...
		if (BuildConfig.DEBUG) {
			...
			Debug.d("EGLCONFIG: " + EGLConfig.class.getSimpleName() + "(Red=" + pConfigChooser.getRedSize() + ", Green="
					+ pConfigChooser.getGreenSize() + ", Blue=" + pConfigChooser.getBlueSize() + ", Alpha=" + pConfigChooser.getAlphaSize()
					+ ", Depth=" + pConfigChooser.getDepthSize() + ", Stencil=" + pConfigChooser.getStencilSize() + ")");
	 ...
	 *            from {@link GLES20#GL_TEXTURE0} to {@link GLES20#GL_TEXTURE31}
	 *            .
	 */
	...
	public void orthoModelViewGLMatrixf(final float pLeft, final float pRight, final float pBottom, final float pTop, final float pZNear,
			final float pZFar) {
	...
	public void orthoProjectionGLMatrixf(final float pLeft, final float pRight, final float pBottom, final float pTop, final float pZNear,
			final float pZFar) {
	...
	public float[] getModelViewProjectionGLMatrix() {
		Matrix.multiplyMM(this.mModelViewProjectionGLMatrix, 0, this.mProjectionGLMatrixStack.mMatrixStack,
				this.mProjectionGLMatrixStack.mMatrixStackOffset, this.mModelViewGLMatrixStack.mMatrixStack,
				this.mModelViewGLMatrixStack.mMatrixStackOffset);
	...
	/**
	 * <b>Note:</b> does not pre-multiply the alpha channel!</br> Except that
	 * difference, same as:
	 * {@link GLUtils#texSubImage2D(int, int, int, int, Bitmap, int, int)}</br>
	 * </br> See topic: '<a href=
	 * "http://groups.google.com/group/android-developers/browse_thread/thread/baa6c33e63f82fca"
	 * >PNG loading that doesn't premultiply alpha?</a>'
	 * 
	 * @param pBorder
	 */
	public void glTexImage2D(final int pTarget, final int pLevel, final Bitmap pBitmap, final int pBorder, final PixelFormat pPixelFormat) {
		...
		GLES20.glTexImage2D(pTarget, pLevel, pPixelFormat.getGLInternalFormat(), pBitmap.getWidth(), pBitmap.getHeight(), pBorder,
				pPixelFormat.getGLFormat(), pPixelFormat.getGLType(), pixelBuffer);
	}
	 ...
	 * >PNG loading that doesn't premultiply alpha?</a>'
	 */
	public void glTexSubImage2D(final int pTarget, final int pLevel, final int pX, final int pY, final Bitmap pBitmap,
			final PixelFormat pPixelFormat) {
		final Buffer pixelBuffer = GLHelper.getPixels(pBitmap, pPixelFormat, ByteOrder.BIG_ENDIAN);

		GLES20.glTexSubImage2D(pTarget, pLevel, pX, pY, pBitmap.getWidth(), pBitmap.getHeight(), pPixelFormat.getGLFormat(),
				pPixelFormat.getGLType(), pixelBuffer);
	}

	/**
	 * Tells the OpenGL driver to send all pending commands to the GPU
	 * immediately.
	 * 
	...
	/**
	 * Tells the OpenGL driver to send all pending commands to the GPU
	 * immediately, and then blocks until the effects of those commands have
	 * been completed on the GPU. Since this is a costly method it should be
	 * only called when really needed.
	 * 
