...
public class GLHelper {
	...
	public static Buffer getPixels(final Bitmap pBitmap, final PixelFormat pPixelFormat, final ByteOrder pByteOrder) {
		...
		switch (pPixelFormat) {
		case RGB_565:
			return ShortBuffer.wrap(GLHelper.convertARGB_8888toRGB_565(pixelsARGB_8888, pByteOrder)); // TODO
																										// Is
																										// ShortBuffer
																										// or
																										// IntBuffer
		...
		case RGBA_4444:
			return ShortBuffer.wrap(GLHelper.convertARGB_8888toRGBA_4444(pixelsARGB_8888, pByteOrder)); // TODO
																										// Is
																										// ShortBuffer
																										// or
																										// IntBuffer
	...
	public static int[] convertARGB_8888toRGBA_8888(final int[] pPixelsARGB_8888, final ByteOrder pByteOrder) {
		if (pByteOrder == ByteOrder.LITTLE_ENDIAN) {
			for (int i = pPixelsARGB_8888.length - 1; i >= 0; i--) {
				...
				/* [A][R][G][B] to [A][B][G][R] */
				/*
				 * From : [ A7 A6 A5 A4 A3 A2 A1 A0 | R7 R6 R5 R4 R3 R2 R1 R0 |
				 * G7 G6 G5 G4 G3 G2 G1 G0 | B7 B6 B5 B4 B3 B2 B1 B0 ]
				 */
				/*
				 * To : [ A7 A6 A5 A4 A3 A2 A1 A0 | B7 B6 B5 B4 B3 B2 B1 B0 | G7
				 * G6 G5 G4 G3 G2 G1 G0 | R7 R6 R5 R4 R3 R2 R1 R0 ]
				 */
			...
			for (int i = pPixelsARGB_8888.length - 1; i >= 0; i--) {
				 ...
				 */
				/*
				 * To : [ R7 R6 R5 R4 R3 R2 R1 R0 | G7 G6 G5 G4 G3 G2 G1 G0 | B7
	...
	public static byte[] convertARGB_8888toA_8(final int[] pPixelsARGB_8888) {
		...
		for (int i = pPixelsARGB_8888.length - 1; i >= 0; i--) {
			/* [A][R][G][B] to [A] */
			/*
			 * From : [ A7 A6 A5 A4 A3 A2 A1 A0 | R7 R6 R5 R4 R3 R2 R1 R0 | G7
			 * G6 G5 G4 G3 G2 G1 G0 | B7 B6 B5 B4 B3 B2 B1 B0 ]
			 */
	...
	public static int[] convertRGBA_8888toARGB_8888(final int[] pPixelsRGBA_8888, final ByteOrder pByteOrder) {
		if (pByteOrder == ByteOrder.LITTLE_ENDIAN) {
			for (int i = pPixelsRGBA_8888.length - 1; i >= 0; i--) {
				...
				/* [A][B][G][R] to [A][R][G][B] */
				/*
				 * From : [ A7 A6 A5 A4 A3 A2 A1 A0 | B7 B6 B5 B4 B3 B2 B1 B0 |
				 * G7 G6 G5 G4 G3 G2 G1 G0 | R7 R6 R5 R4 R3 R2 R1 R0 ]
				 */
				/*
				 * To : [ A7 A6 A5 A4 A3 A2 A1 A0 | R7 R6 R5 R4 R3 R2 R1 R0 | G7
				 * G6 G5 G4 G3 G2 G1 G0 | B7 B6 B5 B4 B3 B2 B1 B0 ]
				 */
			...
			for (int i = pPixelsRGBA_8888.length - 1; i >= 0; i--) {
				...
				/* [R][G][B][A] to [A][R][G][B] */
				/*
				 * From : [ R7 R6 R5 R4 R3 R2 R1 R0 | G7 G6 G5 G4 G3 G2 G1 G0 |
		...
		if (pByteOrder == ByteOrder.LITTLE_ENDIAN) {
			for (int y = 0; y < pHeight; y++) {
				for (int x = 0; x < pWidth; x++) {
					...
					/* [A][B][G][R] to [A][R][G][B] */
					/*
					 * From : [ A7 A6 A5 A4 A3 A2 A1 A0 | B7 B6 B5 B4 B3 B2 B1
					 * B0 | G7 G6 G5 G4 G3 G2 G1 G0 | R7 R6 R5 R4 R3 R2 R1 R0 ]
					 */
					/*
					 * To : [ A7 A6 A5 A4 A3 A2 A1 A0 | R7 R6 R5 R4 R3 R2 R1 R0
					 * | G7 G6 G5 G4 G3 G2 G1 G0 | B7 B6 B5 B4 B3 B2 B1 B0 ]
					 */
					pixelsARGB_8888[x + ((pHeight - y - 1) * pWidth)] = (pixel & 0xFF00FF00) | ((pixel << 16) & 0x00FF0000)
							| ((pixel >> 16) & 0x000000FF);
				}
			}
		} else {
			for (int y = 0; y < pHeight; y++) {
				for (int x = 0; x < pWidth; x++) {
					...
					/* [R][G][B][A] to [A][R][G][B] */
					/*
					 * From : [ R7 R6 R5 R4 R3 R2 R1 R0 | G7 G6 G5 G4 G3 G2 G1
					 * G0 | B7 B6 B5 B4 B3 B2 B1 B0 | A7 A6 A5 A4 A3 A2 A1 A0 ]
					 */
					/*
					 * To : [ A7 A6 A5 A4 A3 A2 A1 A0 | R7 R6 R5 R4 R3 R2 R1 R0
					 * | G7 G6 G5 G4 G3 G2 G1 G0 | B7 B6 B5 B4 B3 B2 B1 B0 ]
