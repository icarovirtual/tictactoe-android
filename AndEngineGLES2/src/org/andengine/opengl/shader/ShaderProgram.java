...
public class ShaderProgram {
	...
	public int getAttributeLocation(final String pAttributeName) {
		...
		if (location != null) {
			...
			throw new ShaderProgramException("Unexpected attribute: '" + pAttributeName + "'. Existing attributes: "
					+ this.mAttributeLocations.toString());
		}
	}
	...
	public int getUniformLocation(final String pUniformName) {
		...
		if (location != null) {
			...
			throw new ShaderProgramException("Unexpected uniform: '" + pUniformName + "'. Existing uniforms: "
					+ this.mUniformLocations.toString());
		}
	}
	...
	public void unbind(final GLState pGLState) throws ShaderProgramException {
		// pGLState.useProgram(0); // TODO Does this have an positive/negative
	...
	private void initUniformLocations() throws ShaderProgramLinkException {
		...
		for (int i = 0; i < numUniforms; i++) {
			GLES20.glGetActiveUniform(this.mProgramID, i, ShaderProgram.NAME_CONTAINER_SIZE, ShaderProgram.LENGTH_CONTAINER, 0,
					ShaderProgram.SIZE_CONTAINER, 0, ShaderProgram.TYPE_CONTAINER, 0, ShaderProgram.NAME_CONTAINER, 0);

			int length = ShaderProgram.LENGTH_CONTAINER[0];

			/*
			 * Some drivers do not report the actual length here, but zero. Then
			...
			if (location == ShaderProgramConstants.LOCATION_INVALID) {
				/*
				 * Some drivers do not report an incorrect length. Then the name
				 * is '\0' terminated.
				 */
				length = 0;
				while (length < ShaderProgram.NAME_CONTAINER_SIZE && ShaderProgram.NAME_CONTAINER[length] != '\0') {
	...
	/**
	 * TODO Is this actually needed? As the locations of
	 * {@link VertexBufferObjectAttribute}s are now 'predefined'.
	 */
	@Deprecated
	private void initAttributeLocations() {
		...
		for (int i = 0; i < numAttributes; i++) {
			GLES20.glGetActiveAttrib(this.mProgramID, i, ShaderProgram.NAME_CONTAINER_SIZE, ShaderProgram.LENGTH_CONTAINER, 0,
					ShaderProgram.SIZE_CONTAINER, 0, ShaderProgram.TYPE_CONTAINER, 0, ShaderProgram.NAME_CONTAINER, 0);
	 ...
	 * @param pUniformName
	 * @param pTexture
	 *            the index of the Texture to use. Similar to
	 *            {@link GLES20#GL_TEXTURE0}, {@link GLES20#GL_TEXTURE1}, ...
	 *            except that it is <b><code>0</code></b> based.
	 */
