...
public class Font implements IFont {
	...
	public Font(final FontManager pFontManager, final ITexture pTexture, final Typeface pTypeface, final float pSize,
	...
	/**
	 * @return the distance from the baseline to the top, which is usually
	 *         negative.
	 */
	...
	/**
	 * @return the distance from the baseline to the bottom, which is usually
	 *         positive.
	 */
		...
		if (whitespace) {
			...
			if ((this.mCurrentTextureY + letterHeight) >= textureHeight) {
				throw new FontException("Not enough space for " + Letter.class.getSimpleName() + ": '" + pCharacter + "' on the "
						+ this.mTexture.getClass().getSimpleName() + ". Existing Letters: "
						+ SparseArrayUtils.toString(this.mManagedCharacterToLetterMap));
			}
			...
			letter = new Letter(pCharacter, this.mCurrentTextureX - Font.LETTER_TEXTURE_PADDING, this.mCurrentTextureY
					- Font.LETTER_TEXTURE_PADDING, letterWidth, letterHeight, letterLeft, letterTop - this.getAscent(), advance, u, v, u2,
					v2);
			...
			if (lettersPendingToBeDrawnToTexture.size() > 0) {
				...
				for (int i = lettersPendingToBeDrawnToTexture.size() - 1; i >= 0; i--) {
					...
					if (!letter.isWhitespace()) {
						...
						final boolean useDefaultAlignment = MathUtils.isPowerOfTwo(bitmap.getWidth())
								&& MathUtils.isPowerOfTwo(bitmap.getHeight()) && (pixelFormat == PixelFormat.RGBA_8888);
