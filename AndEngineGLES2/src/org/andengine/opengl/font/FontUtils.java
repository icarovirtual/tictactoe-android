...
public class FontUtils {
	 ...
	 * @param pEnd
	 *            <code>1</code> beyond the index of the last character to
	 *            measure.
	 * @return the width of pText.
	 */
	 ...
	 * @param pText
	 * @param pWidths
	 *            (optional) If not <code>null</code>, returns the actual width
	 *            measured.
	 * @return the width of pText.
	 */
	 ...
	 * @param pFont
	 * @param pText
	 * @param pMeasureDirection
	 *            If {@link MeasureDirection#FORWARDS}, starts with the first
	 *            character in the {@link CharSequence}. If
	 *            {@link MeasureDirection#BACKWARDS} starts with the last
	 *            character in the {@link CharSequence}.
	 * @param pWidthMaximum
	 * @param pMeasuredWidth
	 *            (optional) If not <code>null</code>, returns the actual width
	 *            measured. Must be an Array of size <code>1</code> or bigger.
	 * @return the number of chars that were measured.
	 */
	public static int breakText(final IFont pFont, final CharSequence pText, final MeasureDirection pMeasureDirection,
			final float pWidthMaximum, final float[] pMeasuredWidth) {
	 ...
	 * @return
	 */
	public static <L extends List<CharSequence>> L splitLines(final IFont pFont, final CharSequence pText, final L pResult,
			final AutoWrap pAutoWrap, final float pAutoWrapWidth) {
		/**
		 * TODO In order to respect already existing linebreaks,
		 * {@link FontUtils#split(CharSequence, List)} could be leveraged and
		 * than the following methods could be called for each line.
		 */
		switch (pAutoWrap) {
		case LETTERS:
	...
	private static <L extends List<CharSequence>> L splitLinesByLetters(final IFont pFont, final CharSequence pText, final L pResult,
			final float pAutoWrapWidth) {
		...
		for (int i = 0; i < textLength; i++) {
			...
			if (charsAvailable) {
				// /* Just for debugging. */
				// final CharSequence line = pText.subSequence(lineStart,
				// lineEnd);
				// final float lineWidth = FontUtils.measureText(pFont, pText,
				// lineStart, lineEnd);
				//
				// final CharSequence lookaheadLine =
				// pText.subSequence(lineStart, lastNonWhitespace);
				...
				if (isEndReached) {
					/*
					 * When the end of the string is reached, add remainder to
					 * result.
					 */
	...
	private static <L extends List<CharSequence>> L splitLinesByWords(final IFont pFont, final CharSequence pText, final L pResult,
			final float pAutoWrapWidth) {
		final int textLength = pText.length();
		...
		while (i < textLength) {
			...
			if (widthNeeded <= lineWidthRemaining) {
				...
				if (firstWordInLine) {
					...
					if (wordEnd == textLength) {
						/* Add the last word. */
						pResult.add(pText.subSequence(wordStart, wordEnd)); // TODO
																			// Does
																			// this
																			// cover
																			// all
	...
	private static <L extends List<CharSequence>> L splitLinesByCJK(final IFont pFont, final CharSequence pText, final L pResult,
			final float pAutoWrapWidth) {
		final int textLength = pText.length();
