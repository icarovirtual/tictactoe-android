...
/**
 * Compared to a {@link HighPerformanceVertexBufferObject} or a
 * {@link LowMemoryVertexBufferObject}, the {@link ZeroMemoryVertexBufferObject}
 * uses <b><u>no</u> permanent heap memory</b>, at the cost of expensive data
 * buffering (<b>up to <u>5x</u> slower!</b>) whenever the bufferdata needs to
 * be updated and higher GC activity, due to the temporary {@link ByteBuffer}
 * allocations.
 * <p/>
 * Usually a {@link ZeroMemoryVertexBufferObject} is preferred to a
 * {@link HighPerformanceVertexBufferObject} or a
 * {@link LowMemoryVertexBufferObject} when the following conditions are met:
 * <ol>
 * <li>The application is close to run out of memory.</li>
 * <li>You have very big {@link HighPerformanceVertexBufferObject}/
 * {@link LowMemoryVertexBufferObject} or an extreme number of small
 * {@link HighPerformanceVertexBufferObject}/{@link LowMemoryVertexBufferObject}
 * s, where you can't afford to have any of the bufferdata to be kept in heap
 * memory.</li>
 * <li>The content (color, vertices, texturecoordinates) of the
 * {@link ZeroMemoryVertexBufferObject} is changed not often, or even better:
 * never.</li>
 * </ol>
 * <p/>
 * (c) Zynga 2011
...
public abstract class ZeroMemoryVertexBufferObject implements IVertexBufferObject {
	...
	public ZeroMemoryVertexBufferObject(final VertexBufferObjectManager pVertexBufferObjectManager, final int pCapacity,
			final DrawType pDrawType, final boolean pAutoDispose, final VertexBufferObjectAttributes pVertexBufferObjectAttributes) {
	...
	public void unbind(final GLState pGLState, final ShaderProgram pShaderProgram) {
		...
		// pGLState.bindBuffer(0); // TODO Does this have an positive/negative
	...
	/**
	 * When a non <code>null</code> {@link ByteBuffer} is returned by this
	 * function, it is guaranteed that
	 * {@link ZeroMemoryVertexBufferObject#releaseByteBuffer(ByteBuffer)} is
	 * called.
	 * 
	 * @return a {@link ByteBuffer} to be passed to
	 *         {@link ZeroMemoryVertexBufferObject#onPopulateBufferData(ByteBuffer)}
	 *         .
	 */
