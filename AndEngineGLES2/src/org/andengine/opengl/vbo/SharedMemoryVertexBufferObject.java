...
/**
 * Compared to {@link ZeroMemoryVertexBufferObject}, all
 * {@link SharedMemoryVertexBufferObject}s share a single {@link ByteBuffer}
 * which is used by whichever {@link SharedMemoryVertexBufferObject} instance is
 * currently buffering data, at the cost of expensive data buffering (<b>up to
 * <u>5x</u> slower!</b>) and a little synchronization overhead.
 * <p/>
 * Usually a {@link SharedMemoryVertexBufferObject} is preferred to a
 * {@link ZeroMemoryVertexBufferObject} when the following conditions need to be
 * met:
 * <ol>
...
public abstract class SharedMemoryVertexBufferObject extends ZeroMemoryVertexBufferObject {
	...
	public SharedMemoryVertexBufferObject(final VertexBufferObjectManager pVertexBufferObjectManager, final int pCapacity,
			final DrawType pDrawType, final VertexBufferObjectAttributes pVertexBufferObjectAttributes) {
	...
	protected ByteBuffer aquireByteBuffer() {
		...
		if (SharedMemoryVertexBufferObject.sSharedByteBuffer == null
				|| SharedMemoryVertexBufferObject.sSharedByteBuffer.capacity() < byteCapacity) {
			if (SharedMemoryVertexBufferObject.sSharedByteBuffer != null) {
				BufferUtils.freeDirectByteBuffer(SharedMemoryVertexBufferObject.sSharedByteBuffer);
			}
