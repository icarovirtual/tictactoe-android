...
/**
 * The general workflow with a {@link RenderTexture} is:
 * {@link RenderTexture#init(GLState)} -> {@link RenderTexture#begin(GLState)}
 * -> {@link RenderTexture#end(GLState)} ->
 * {@link RenderTexture#destroy(GLState)}.
 * 
 * (c) Zynga 2011
 * 
...
public class RenderTexture extends Texture {
	...
	public RenderTexture(final TextureManager pTextureManager, final int pWidth, final int pHeight, final PixelFormat pPixelFormat,
			final TextureOptions pTextureOptions, final ITextureStateListener pTextureStateListener) {
	...
	protected void writeTextureToHardware(final GLState pGLState) {
		GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, this.mPixelFormat.getGLInternalFormat(), this.mWidth, this.mHeight, 0,
				this.mPixelFormat.getGLFormat(), this.mPixelFormat.getGLType(), null);
	}
	 ...
	 *             when this {@link RenderTexture} could not be initialized. The
	 *             {@link GLException} contains the error code. When this
	 *             exception is throw, all cleanup will be automatically
	 *             performed through {@link RenderTexture#destroy(GLState)}.
	 */
	...
	/**
	 * @see {@link RenderTexture#end(GLState)}, {@link
	 *      RenderTexture#end(GLState, boolean, boolean} .
	 */
	 ...
	 * 
	 * @param pColor
	 *            the {@link Color} to clear this {@link RenderTexture}.
	 */
	 ...
	 * 
	 * @param pRed
	 *            the red portion of the color to clear this
	 *            {@link RenderTexture}.
	 * @param pGreen
	 *            the green portion of the color to clear this
	 *            {@link RenderTexture}.
	 * @param pBlue
	 *            the blue portion of the color to clear this
	 *            {@link RenderTexture}.
	 * @param pAlpha
	 *            the alpha portion of the color to clear this
	 *            {@link RenderTexture}.
	 */
	 ...
	 *            {@link RenderTexture}.
	 */
	public void begin(final GLState pGLState, final boolean pFlipX, final boolean pFlipY, final float pRed, final float pGreen,
		...
		/* Restore clear color. */
		GLES20.glClearColor(RenderTexture.CLEARCOLOR_CONTAINER[RenderTexture.CLEARCOLOR_CONTAINER_RED_INDEX],
				RenderTexture.CLEARCOLOR_CONTAINER[RenderTexture.CLEARCOLOR_CONTAINER_GREEN_INDEX],
				RenderTexture.CLEARCOLOR_CONTAINER[RenderTexture.CLEARCOLOR_CONTAINER_BLUE_INDEX],
				RenderTexture.CLEARCOLOR_CONTAINER[RenderTexture.CLEARCOLOR_CONTAINER_ALPHA_INDEX]);
	}
	 ...
	 * @param pGLState
	 * @param pFlush
	 *            {@link GLState#flush()} has lower preference than pFinish.
	 * @param pFinish
	 *            {@link GLState#finish()} had higher preference than pFlush.
	 * 
	 * @see {@link RenderTexture#begin(GLState)},
	 *      {@link RenderTexture#begin(GLState, boolean, boolean)},
	 *      {@link RenderTexture#begin(GLState, Color)},
	...
	public Bitmap getBitmap(final GLState pGLState, final int pX, final int pY, final int pWidth, final int pHeight) {
		if (this.mPixelFormat != PixelFormat.RGBA_8888) {
			throw new IllegalStateException("Currently only 'PixelFormat." + PixelFormat.RGBA_8888
					+ "' is supported to be retrieved as a Bitmap.");
		}
