...
public abstract class PVRTexture extends Texture {
	...
	public static final int FLAG_BUMPMAP = (1 << 10); // has normals encoded for
														// a bump map
	public static final int FLAG_TILING = (1 << 11); // is bordered for tiled
														// pvr
	public static final int FLAG_CUBEMAP = (1 << 12); // is a cubemap/skybox
	public static final int FLAG_FALSEMIPCOL = (1 << 13); // are there false
															// colored MIP
															// levels
	public static final int FLAG_VOLUME = (1 << 14); // is this a volume texture
	public static final int FLAG_ALPHA = (1 << 15); // v2.1 is there
													// transparency info in the
													// texture
	public static final int FLAG_VERTICALFLIP = (1 << 16); // v2.1 is the
															// texture
															// vertically
															// flipped
	...
	public PVRTexture(final TextureManager pTextureManager, final PVRTextureFormat pPVRTextureFormat) throws IllegalArgumentException,
			IOException {
		this(pTextureManager, pPVRTextureFormat, new GreedyPVRTexturePixelBufferStrategy(), TextureOptions.DEFAULT, null);
	}

	public PVRTexture(final TextureManager pTextureManager, final PVRTextureFormat pPVRTextureFormat,
			final IPVRTexturePixelBufferStrategy pPVRTexturePixelBufferStrategy) throws IllegalArgumentException, IOException {
		this(pTextureManager, pPVRTextureFormat, pPVRTexturePixelBufferStrategy, TextureOptions.DEFAULT, null);
	}

	public PVRTexture(final TextureManager pTextureManager, final PVRTextureFormat pPVRTextureFormat,
			final ITextureStateListener pTextureStateListener) throws IllegalArgumentException, IOException {
	...
	public PVRTexture(final TextureManager pTextureManager, final PVRTextureFormat pPVRTextureFormat,
			final IPVRTexturePixelBufferStrategy pPVRTexturePixelBufferStrategy, final ITextureStateListener pTextureStateListener)
	...
	public PVRTexture(final TextureManager pTextureManager, final PVRTextureFormat pPVRTextureFormat, final TextureOptions pTextureOptions)
			throws IllegalArgumentException, IOException {
		this(pTextureManager, pPVRTextureFormat, new GreedyPVRTexturePixelBufferStrategy(), pTextureOptions, null);
	}

	public PVRTexture(final TextureManager pTextureManager, final PVRTextureFormat pPVRTextureFormat,
			final IPVRTexturePixelBufferStrategy pPVRTexturePixelBufferStrategy, final TextureOptions pTextureOptions)
			throws IllegalArgumentException, IOException {
	...
	public PVRTexture(final TextureManager pTextureManager, final PVRTextureFormat pPVRTextureFormat,
		...
		if (this.mPVRTextureHeader.getPVRTextureFormat().getPixelFormat() != pPVRTextureFormat.getPixelFormat()) {
			throw new IllegalArgumentException("Other PVRTextureFormat: '" + this.mPVRTextureHeader.getPVRTextureFormat().getPixelFormat()
					+ "' found than expected: '" + pPVRTextureFormat.getPixelFormat() + "'.");
		}

		if (this.mPVRTextureHeader.getPVRTextureFormat().isCompressed()) { // TODO
																			// &&
																			// !
																			// GLHELPER_EXTENSION_PVRTC]
																			// )
																			// {
		...
		if (this.hasMipMaps()) {
			...
			default:
				if (BuildConfig.DEBUG) {
					Debug.w("This '" + this.getClass().getSimpleName() + "' contains mipmaps, but the provided '"
							+ pTextureOptions.getClass().getSimpleName() + "' don't have MipMaps enabled on the MinFilter!");
				}
			}
		}
	...
	protected void writeTextureToHardware(final GLState pGLState) throws IOException {
		final IPVRTexturePixelBufferStrategyBufferManager pvrTextureLoadStrategyManager = this.mPVRTexturePixelBufferStrategy
				.newPVRTexturePixelBufferStrategyManager(this);
		...
		while (currentPixelDataOffset < dataLength) {
			if (currentLevel > 0 && (width != height || MathUtils.nextPowerOfTwo(width) != width)) {
				Debug.w("Mipmap level '" + currentLevel + "' is not squared. Width: '" + width + "', height: '" + height
			...
			this.mPVRTexturePixelBufferStrategy.loadPVRTextureData(pvrTextureLoadStrategyManager, width, height, bytesPerPixel,
					this.mPixelFormat, currentLevel, currentPixelDataOffset, currentPixelDataSize);
	...
	public ByteBuffer getPVRTextureBuffer() throws IOException {
		...
		try {
			final ByteBufferOutputStream os = new ByteBufferOutputStream(DataConstants.BYTES_PER_KILOBYTE,
					DataConstants.BYTES_PER_MEGABYTE / 2);
	...
	public static class PVRTextureHeader {
		...
		static final byte[] MAGIC_IDENTIFIER = { (byte) 'P', (byte) 'V', (byte) 'R', (byte) '!' };
		...
		public PVRTextureHeader(final byte[] pData) {
			...
			/* Check magic bytes. */
			if (!ArrayUtils.equals(pData, 11 * DataConstants.BYTES_PER_INT, PVRTextureHeader.MAGIC_IDENTIFIER, 0,
		...
		public int headerLength() {
			return this.mDataByteBuffer.getInt(0 * DataConstants.BYTES_PER_INT); // TODO
																					// Constants
		}
	...
	public static enum PVRTextureFormat {
			...
			// PVRTC_2(0x18, GL10.GL_COMPRESSED_RGBA_PVRTC_2BPPV1_IMG, true,
			// TextureFormat.???),
			// PVRTC_4(0x19, GL10.GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG, true,
			// TextureFormat.???),
