...
public class BitmapTextureAtlas extends TextureAtlas<IBitmapTextureAtlasSource> {
	 ...
	 *            use {@link BitmapTextureFormat#RGBA_8888} or
	 ...
	 *            to be informed when this {@link BitmapTextureAtlas} is loaded,
	 *            unloaded or a {@link ITextureAtlasSource} failed to load.
	 */
	public BitmapTextureAtlas(final TextureManager pTextureManager, final int pWidth, final int pHeight,
			final ITextureAtlasStateListener<IBitmapTextureAtlasSource> pTextureAtlasStateListener) {
	 ...
	 *            the (quality) settings of this {@link BitmapTextureAtlas}.
	 */
	public BitmapTextureAtlas(final TextureManager pTextureManager, final int pWidth, final int pHeight,
	 ...
	 */
	public BitmapTextureAtlas(final TextureManager pTextureManager, final int pWidth, final int pHeight,
			final BitmapTextureFormat pBitmapTextureFormat, final TextureOptions pTextureOptions) throws IllegalArgumentException {
	 ...
	 */
	public BitmapTextureAtlas(final TextureManager pTextureManager, final int pWidth, final int pHeight,
			final TextureOptions pTextureOptions, final ITextureAtlasStateListener<IBitmapTextureAtlasSource> pTextureAtlasStateListener)
	...
	public BitmapTextureAtlas(final TextureManager pTextureManager, final int pWidth, final int pHeight,
			...
			final ITextureAtlasStateListener<IBitmapTextureAtlasSource> pTextureAtlasStateListener) throws IllegalArgumentException {
	...
	protected void writeTextureToHardware(final GLState pGLState) {
		...
		/*
		 * Non alpha premultiplied bitmaps are loaded with ARGB_8888 and
		 * converted down manually.
		 */
		...
		for (int i = 0; i < textureSourceCount; i++) {
			...
			try {
				...
				if (bitmap == null) {
					throw new NullBitmapException("Caused by: " + bitmapTextureAtlasSource.getClass().toString() + " --> "
							+ bitmapTextureAtlasSource.toString() + " returned a null Bitmap.");
				}

				final boolean useDefaultAlignment = MathUtils.isPowerOfTwo(bitmap.getWidth()) && MathUtils.isPowerOfTwo(bitmap.getHeight())
				...
				if (preMultipyAlpha) {
					GLUtils.texSubImage2D(GLES20.GL_TEXTURE_2D, 0, bitmapTextureAtlasSource.getTextureX(),
							bitmapTextureAtlasSource.getTextureY(), bitmap, glFormat, glType);
				} else {
					pGLState.glTexSubImage2D(GLES20.GL_TEXTURE_2D, 0, bitmapTextureAtlasSource.getTextureX(),
							bitmapTextureAtlasSource.getTextureY(), bitmap, this.mPixelFormat);
				}
