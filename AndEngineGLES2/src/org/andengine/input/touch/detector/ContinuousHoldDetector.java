...
/**
 * Note: Needs to be registered as an {@link IUpdateHandler} to the
 * {@link Engine} or {@link Scene} to work properly.
 * 
...
public class ContinuousHoldDetector extends HoldDetector implements IUpdateHandler {
	...
	public ContinuousHoldDetector(final IHoldDetectorListener pHoldDetectorListener) {
		this(HoldDetector.TRIGGER_HOLD_MINIMUM_MILLISECONDS_DEFAULT, HoldDetector.TRIGGER_HOLD_MAXIMUM_DISTANCE_DEFAULT,
				ContinuousHoldDetector.TIME_BETWEEN_UPDATES_DEFAULT, pHoldDetectorListener);
	}

	public ContinuousHoldDetector(final long pTriggerHoldMinimumMilliseconds, final float pTriggerHoldMaximumDistance,
			final float pTimeBetweenUpdates, final IHoldDetectorListener pHoldDetectorListener) {
	...
	/**
	 * When {@link ContinuousHoldDetector#isHolding()} this method will call
	 * through to
		...
		case TouchEvent.ACTION_MOVE: {
			if (this.mPointerID == pSceneTouchEvent.getPointerID()) {
				...
				this.mMaximumDistanceExceeded = this.mMaximumDistanceExceeded
						|| Math.abs(this.mDownX - motionEvent.getX()) > this.mTriggerHoldMaximumDistance
						|| Math.abs(this.mDownY - motionEvent.getY()) > this.mTriggerHoldMaximumDistance;
				return true;
			} else {
				return false;
			}
