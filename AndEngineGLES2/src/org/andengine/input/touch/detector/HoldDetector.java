...
public class HoldDetector extends BaseDetector {
	...
	public HoldDetector(final IHoldDetectorListener pHoldDetectorListener) {
		this(HoldDetector.TRIGGER_HOLD_MINIMUM_MILLISECONDS_DEFAULT, HoldDetector.TRIGGER_HOLD_MAXIMUM_DISTANCE_DEFAULT,
				pHoldDetectorListener);
	}

	public HoldDetector(final long pTriggerHoldMinimumMilliseconds, final float pTriggerHoldMaximumDistance,
			final IHoldDetectorListener pHoldDetectorListener) {
	...
	/**
	 * When {@link HoldDetector#isHolding()} this method will call through to
	 * {@link IHoldDetectorListener#onHoldFinished(HoldDetector, long, int, float, float)
	 * .
	 */
	@Override
	public void reset() {
			...
			if (this.mPointerID == pSceneTouchEvent.getPointerID()) {
				...
				if (holdTimeMilliseconds >= this.mTriggerHoldMinimumMilliseconds) {
					if (this.mTriggering) {
						...
						this.mMaximumDistanceExceeded = this.mMaximumDistanceExceeded
								|| Math.abs(this.mDownX - motionEvent.getX()) > triggerHoldMaximumDistance
								|| Math.abs(this.mDownY - motionEvent.getY()) > triggerHoldMaximumDistance;
	...
	public static interface IHoldDetectorListener {
		...
		public void onHold(final HoldDetector pHoldDetector, final long pHoldTimeMilliseconds, final int pPointerID, final float pHoldX,
				final float pHoldY);

		public void onHoldFinished(final HoldDetector pHoldDetector, final long pHoldTimeMilliseconds, final int pPointerID,
