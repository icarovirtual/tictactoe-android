package apps.game.tictactoe.gameplay;
...
public class IconsController
{
	...
	private int boardPositions[][] = new int[3][3];
	...
	/** 	CHECK THESE RESULTS: </br>
	 *	 x |   |   		x | x | x		  |   | x 		  |   |   </br>
	 *	 x |   | 		  |   |   		  |   | x		  |   |   </br>
	 *	 x |   |		  |   |   		  |   |	x		x | x | x </br>
	 **/
	private boolean checkCorners(int posLine, int posColumn, int boardIcon,
			int incLine, int incColumn)
	{
		if(posColumn < boardPositions.length && posLine < boardPositions.length
				&& posColumn >= 0 && posLine >= 0)
		{
			if(boardPositions[posLine][posColumn] == boardIcon)
			{
				return checkCorners(posLine +incLine, posColumn +incColumn, boardIcon,
						incLine, incColumn);
			}
			else
				return false;
	 ...
	 *	  |   |	x		x |   |   </br>
	**/
	private boolean checkDiagonals(int posLine, int posColumn, int boardIcon,
			int incLine, int incColumn)
	{
		boolean validTop = false;
		boolean validBottom = false;
		validTop = checkDiagonalsAux(posLine, posColumn,
				boardIcon, incLine, incColumn);
		validBottom = checkDiagonalsAux(posLine, posColumn,
				boardIcon, incLine *-1, incColumn *-1);
		return validTop && validBottom;
	}

	private boolean checkDiagonalsAux(int posLine, int posColumn, int boardIcon,
			int incLine, int incColumn)
	{
		if(posColumn < boardPositions.length && posLine < boardPositions.length
				&& posColumn >= 0 && posLine >= 0)
		{
			if(boardPositions[posLine][posColumn] == boardIcon)
			{
				...
				validGame = checkDiagonalsAux(posLine +incLine, posColumn +incColumn,
						boardIcon, incLine, incColumn);
				return validGame;
			}
			else
	 ...
	 *	  |   |	 		  | x |   </br>
	**/
	private boolean checkCross(int posLine, int posColumn, int boardIcon,
			int incLine, int incColumn)
	{
		boolean validHigh = false;
		boolean validLow = false;
		validHigh = checkCrossAux(posLine, posColumn,
				boardIcon, incLine, incColumn);
		validLow = checkCrossAux(posLine, posColumn,
				boardIcon, incLine *-1, incColumn *-1);
		return validHigh && validLow;
	}

	private boolean checkCrossAux(int posLine, int posColumn, int boardIcon,
			int incLine, int incColumn)
	{
		if(posColumn < boardPositions.length && posLine < boardPositions.length
				&& posColumn >= 0 && posLine >= 0)
		{
			...
			{
				boolean validGame = false;
				validGame = checkCrossAux(posLine +incLine, posColumn +incColumn,
					boardIcon, incLine, incColumn);
				return validGame;
			}
			else
